...
function mysqldatetime_to_vector($start, $end=NULL)
{
	...
	$diff = abs(mysqldatetime_to_timestamp($start) - mysqldatetime_to_timestamp($end));
	
	// is distance less than a minute
	if(abs($diff) < (60))
	{
		// return hours
		$ammount = round(($diff));
	...
	// is distance less than an hour
	else if(abs($diff) < (60 * 60))
	{
		// return hours
		$ammount = round(($diff/(60)));
		$units = 'minute';
	}
	// is distance less than an day
	else if(abs($diff) < (24 * 60 * 60))
	{
		// return hours
		$ammount = round(($diff/(60 * 60)));
		$units = 'hour';
	}
	...
	{
		// return months
		$ammount = round(($diff/(30 * 24 * 60 * 60)));
		$units = 'month';
	}
	...
	{
		// return days
		$ammount = round(($diff/(365 * 24 * 60 * 60)));
		$units = 'year';
	}
	
	//log_message('error', $start.' - '.$end.' diff = '.$diff.'  '.$ammount.' '.pluralise($ammount, $units));
	
	return pluralise($ammount, $units);
}
